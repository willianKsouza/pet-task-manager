...
class CreateTaskControllerTest extends TestCase
{
    ...
    public function test_return_json_with_valid_data()
    {
        ...
        $response
            ->assertStatus(201)
            ->assertJson(
                function (AssertableJson $json) use ($user, $body) {
                    $json
                        ->where('data.id', 1)
                        ->where('data.title', $body['title'])
                        ->where('data.description', $body['description'])
                        ->where('data.due_date', $body['due_date'])
    ...
    public function test_exception_with_invalid_data(): void
    {
        ...
        $response
            ...
            ->assertJsonPath('error.validation_errors.0.field', 'title')
            ->assertJsonPath('error.validation_errors.1.field', 'description')
            ->assertJsonPath('error.validation_errors.2.field', fn (null $field) => $field == null);
    }
    ...
    public function test_verify_is_autenticated(): void
    {
        $user = User::factory()->create();
    ...
    public function test_verify_if_service_is_called(): void
    {
        $user = User::factory()->create();

        $this->actingAs($user);

        $future_date = Carbon::now()->addDays(1)->format('Y-m-d');
        ...
        $this->instance(
            ...
            Mockery::mock(CreateTaskService::class, function (MockInterface $mock) use ($expectedTask) {
